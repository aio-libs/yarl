...
- **Agent output goes in a footer, below the PR summary.** The
  template sections (What / Are there changes in behavior / etc.)
  come first and read like a human wrote them. Anything the
  agent wants to surface for reviewers (scan results, test logs,
  branch hygiene notes) goes underneath that, clearly separated.
  A collapsed `<details>` block at the very bottom is the
  preferred shape, so it does not crowd the summary:
  ...
  sections, putting it above the summary, or dressing it up with
  promotional branding. The closed PR at
  `aio-libs/multidict#1336` ended with the block below; the
  problem was not its position, it was the "Quality Report"
  heading and the "Generated by <pipeline name> post-mission
  quality pipeline" tagline:
  ...
  *Generated by <pipeline name> post-mission quality pipeline*
  ```

  Same data in a plain `<details>` block, without the branded
  heading and tagline, would be fine.
