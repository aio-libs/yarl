...
### 4. Do not advertise the agent

- No `Co-Authored-By:` trailer naming Claude, an LLM, or any AI
  tool, in commits or in the PR body.
- No "Generated by", "Quality Report", "Test summary by
  <agent>", or similar footers in the PR body. The closed
  `aio-libs/multidict#1336` had exactly this footer and it was
  the giveaway that closed the PR.
...
- The repo does **not** use Conventional Commits as a CI gate
  (unlike `dbus-fast`). Recent landed subjects are short
  imperative or descriptive prose (e.g. `Reject URLs with text
  before bracket in host`, `ci: switch typing coverage tracking
  to Coveralls`, `docs: note UUID-to-int coercion in with_query
  docs`). Match that style; do not force `feat:` / `fix:`
  prefixes onto every commit.
